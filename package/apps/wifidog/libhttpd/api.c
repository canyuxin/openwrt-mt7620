...
// called by main_loop(), 创建一个tcp服务监听2060端口(默认的)
...
// called by main(), wifidog的http服务器接受(accept)一个客户端连接
	...
	struct	timeval *timeout;
{
	...
	struct  sockaddr_in     addr;
	size_t  addrLen;
	char	ipaddr[24];  // modified by lijg, 2013-07-25 
	request	*r;
	...
	while(result == 0)
	{
		...
		//fprintf(stderr, "call select return %d\n", result);
		if (result < 0)
		{
			server->lastError = -1; //debug(LOG_INFO, "call select return %d, %s", result, strerror(errno));
			return(NULL);
		}
		if (timeout != 0 && result == 0)
		{
			server->lastError = 0;  // modified by lijg, 2013-08-19, change the position
			return(NULL);
		}
		if (result > 0)
		{
	...
	server->lastError = 0; // add by lijg, 2013-06-04, async refreshing it
	/* Get on with it */
	bzero(&addr, sizeof(addr));
	addrLen = sizeof(addr);
	...
	// modified by lijg, 2013-07-25, because inet_ntoa() is not thread safe, so cancel it
	net_to_str(addr.sin_addr.s_addr, ipaddr, sizeof(ipaddr));
	
	if (r->clientSock > 0) { // modified by lijg, 2013-08-19
		strncpy(r->clientAddr, ipaddr, HTTP_IP_ADDR_LEN-1);
...
// called by thread_httpd(), 从客户端读取一个完整的http请求
int httpdReadRequest(httpd *server, request *r)
{
	...
	*/
	count = 0;
	inHeaders = 1;
	while(_httpd_readLine(r, buf, HTTP_MAX_LEN-4) > 0) //modified by lijg, 2013-06-26,
	{
		count++;

		//buf[HTTP_MAX_LEN-1] = 0;
		//buf[HTTP_MAX_LEN-2] = 0;
		//buf[HTTP_MAX_LEN-3] = 0;
		buf[HTTP_MAX_LEN-4] = 0;  // add by lijg, 2013-08-19, for fixed 
		fprintf(stdout, "%s httpd read line [%s]\n", r->clientAddr, buf); // add by lijg, 2013-06-26, for debug
		...
		if (count == 1)
		{
			...
			*/
			...
			while(isalpha((unsigned char)*cp2))
				cp2++;
			char _bak ;  // add by lijg, 2013-06-26
			_bak = *cp2;
			*cp2 = 0;
			...
			if (_bak == 0) {  // string end
				fprintf(stdout, "%s not found path \n", r->clientAddr); // add by lijg, 2013-06-26, for debug
				return -1;
			}
			...
			while(*cp2 != ' ' && *cp2 != 0)
				cp2++;
			*cp2 = 0;
			strncpy(r->request.path,cp,HTTP_MAX_URL-1); // by lijg , 2013-06-26
...
// modified by lijg , 2013-08-19 , comment it 
#if 0
...
void httpdEndRequest(request *r)
{
	...
	//shutdown(r->clientSock,2); // modified by lijg, 2013-06-01, cancel it for avoiding CLOSE_WAIT status
	close(r->clientSock);
	free(r);
	//fprintf(stderr, " call httpdEndRequest\n\n");
}
...
// called by main_loop(), @function=http_callback_404
// 初始化 http服务的 ->handle404 结构以及其回调函数
int httpdAddC404Content(server, function)
	httpd	*server;
	void	(*function)();
{
	if (!server->handle404) {  // 分配 http404类型结构
...
// 1. called by main_loop(), @dir="/", @name="wifidog", @indexFlag=0, @preload=NULL,
//   @function=http_callback_wifidog, 初始化根目录(@server->content)的回调处理函数
// 2. @dir="/wifidog", @name="", @indexFlag=0, @preload=NULL,@function=http_callback_wifidog,
//  创建初始化子目录wifidog/ 其回调处理函数为 http_callback_wifidog
// 3. @dir="/wifidog", @name="about", @indexFlag=0, @preload=NULL,@function=http_callback_about,
	...
	void	(*function)();
{
    ...
    //1.1返回@server->content 根目录结构
    //2.1返回创建的子目录(wifidog/)的目录结构体
    //3.1返回2.1创建的子目录(wifidog)的目录结构
    dirPtr = _httpd_findContentDir(server, dir, HTTP_TRUE);
	newEntry =  malloc(sizeof(httpContent));
	if (newEntry == NULL)
		return(-1);
	// 1.2 分配一个 httpContent类型结构, 名为"wifidog",将回调函数 @function 赋予它,
	//  将其挂到 @dirPtr->entries链表头
	// 3.2分配一个...,名为"about",将回调函数 http_callback_about 赋予它, 将其挂到
	//  wifidog目录的->entries链表头
	bzero(newEntry,sizeof(httpContent));
	newEntry->name = strdup(name);
...
{
	strncpy(r->response.response, msg, HTTP_MAX_URL-1); // modified by lijg, 2013-08-19, fix it
...
{
	const char *src;
	char	buf[HTTP_MAX_LEN+4],
		varName[80+4],
		*dest;  // modified by lijg, 2013-08-19, fix it
	int	count;
...
// called by thread_httpd()，处理一个http客户端的请求(可能是防火墙重定向过来的http请求)
void httpdProcessRequest(httpd *server, request *r)
{
	...
	strncpy(dirName, httpdRequestPath(r), HTTP_MAX_URL-1); // modified by lijg, 2013-08-19, fix it
	...
	strncpy(entryName, cp + 1, HTTP_MAX_URL-1);  //modified by lijg, 2013-08-19, fix it
	entryName[HTTP_MAX_URL-1]=0;
	if (cp != dirName)
		*cp = 0;
	else
		*(cp+1) = 0;
	dir = _httpd_findContentDir(server, dirName, HTTP_FALSE); //查找http请求的PATH
	if (dir == NULL)
	{
