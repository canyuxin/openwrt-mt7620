...
time_t started_time = 0; // wifidog进程启动时间
 ...
 */
//从原有wifidog进程中获取已经认证通过的客户端链表信息, called by main() 重启新的wifidog进程
	...
	while (read(sock, &onechar, 1) == 1) {
		...
		if (!onechar) {
			...
			if (client) {
				...
				client_num ++; // add by lijg, 2013-06-04
			}
...
sigchld_handler(int s)
{
	...
	// modified by lijg, 2013-06-04, 防止信号中处理函数中的死锁
...
// called by wdctl_stop(), SIGINT信号处理函数
void
...
main_loop(void)
{
	...
	if (!started_time) {
		...
		started_time = time(NULL);  // 更新wifidog启动时间
	}
	...
	if (!config->gw_id) {  // 设置网关ID为br-lan的MAC地址
	...
	// 分配一个 httpd类型的结构体 => webserver,创建一个http服务
	...
	// 设置http服务器根目录("/")回调处理函数为 @http_callback_wifidog
	httpdAddCContent(webserver, "/", "wifidog", 0, NULL, http_callback_wifidog);
	// 创建初始化子目录wifidog/ 其回调处理函数为 http_callback_wifidog
	httpdAddCContent(webserver, "/wifidog", "", 0, NULL, http_callback_wifidog);
	// 同理创建初始化"about" 子目录 及其回调处理函数
	httpdAddCContent(webserver, "/wifidog", "about", 0, NULL, http_callback_about);
	// 同理创建初始化"status" 子目录 及其回调处理函数
	httpdAddCContent(webserver, "/wifidog", "status", 0, NULL, http_callback_status);
	// 同理创建初始化"auth" 子目录 及其回调处理函数
	httpdAddCContent(webserver, "/wifidog", "auth", 0, NULL, http_callback_auth);
	//Add splash to allow the access of Internet
	httpdAddCContent(webserver, "/cgi-bin", "splash", 0, NULL, http_callback_splash);

	httpdAddC404Content(webserver, http_callback_404); // 初始化 http服务的 ->handle404 结构以及其回调函数

	/* Reset the firewall (if WiFiDog crashed) */
	fw_destroy(); // 调用 iptables_fw_destroy()
	/* Then initialize it */
	if (!fw_init()) {
	...
	///* // modified by lijg, 2013-06-04 , for test
	...
	while(1) {
		// add by lijg, 2013-06-04, for test
		//sleep(10);
		//continue;
		///////////////////////////////////////////

		// modified by lijg, 2013-06-04
		struct timeval _tval;
		_tval.tv_sec = 10;
		_tval.tv_usec = 0;
		r = httpdGetConnection(webserver, &_tval);
		//r = httpdGetConnection(webserver, NULL);
		fprintf(stderr, "http get connect return %p, %d\n", r, webserver->lastError);
		...
		if (webserver->lastError == -1) {
			...
			*params = webserver;  // wifidog http服务器结构
			*(params + 1) = r;    // http客户端结构

            // wifidog的web服务器创建一个线程 去处理一个http客户端的请求
...
// add by lijg, 2013-06-04 , for generate core dump file
#include <sys/time.h>
...
int main(int argc, char **argv) {
	...
	s_config *config = config_get_config();  //返回全局配置结构的指针
	config_init();  // 初始化默认的配置项的值

	parse_commandline(argc, argv);

	/* Initialize the config */
	config_read(config->configfile);  // 从配置文件中导入配置项
	config_validate(); //检查配置项, "GatewayInterface", "AuthServer" 必须配置

	/* Initializes the linked list of connected clients */
	client_list_init();

	/* Init the signals to catch chld/quit/etc */
	init_signals();  // 初始化忽略一些系统信号

	// add by lijg, 2013-06-04 , for generate core dump file
	setcoredump();

	if (restart_orig_pid) { // 重启wifidog
		/*
		 * We were restarted and our parent is waiting for us to talk to it over the socket
		 */
		get_clients_from_parent(); //从原有wifidog进程中获取已经认证通过的客户端链表信息
	...
	if (config->daemon) {
		...
		switch(safe_fork()) {  // 进入后台运行wifidog进程
			case 0: /* child */
				setsid();
