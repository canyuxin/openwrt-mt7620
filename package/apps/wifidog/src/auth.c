...
*/
// 线程入口函数 called by main() int gateway.c
// 定时检查认证通过的客户端 是否存在异常
void
thread_client_timeout_check(const void *arg)
{
	...
	while (1) {
		...
		timeout.tv_sec = time(NULL) + config_get_config()->checkinterval; // = 60秒
		timeout.tv_nsec = 0;
		...
		/* Thread safe "sleep" */
		pthread_cond_timedwait(&cond, &cond_mutex, &timeout); //定时60秒

		/* No longer needs to be locked */
		pthread_mutex_unlock(&cond_mutex); // 间隔60s调用一次
...
*///网关向认证服务器发起http验证请求(验证客户端发来的token是否正确), called by http_callback_auth()
void
authenticate_client(request *r)
{
    ...
    case AUTH_VALIDATION:
		...
		client_fwrule_save(client->ip, client->mac, client->token);  // add by lijg, 2013-08-13
		safe_asprintf(&urlFragment, "%smessage=%s",
			auth_server->authserv_msg_script_path_fragment,
    ...
    case AUTH_ALLOWED:
		...
		fw_allow(client->ip, client->mac, FW_MARK_KNOWN); //添加防火墙规则,允许该客户端访问互联网
		client_fwrule_save(client->ip, client->mac, client->token);  // add by lijg, 2013-08-13
		served_this_session++;
