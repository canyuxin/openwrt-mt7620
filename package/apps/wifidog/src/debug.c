...
_debug(char *filename, int line, int level, char *format, ...)
{
    ...
    if (config->debuglevel >= level) {
        ...
        if (level <= LOG_WARNING) {
            fprintf(stderr, "[%d][%.24s][%u](%s:%d) ", level, ctime_r(&ts, buf), pthread_self(),
			    filename, line);  // modified by lijg, 2013-05-18
            ...
            fprintf(stdout, "[%d][%.24s][%u](%s:%d) ", level, ctime_r(&ts, buf), pthread_self(),
			    filename, line); // modified by lijg, 2013-05-18
            va_start(vlist, format);
