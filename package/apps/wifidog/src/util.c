...
/*
 * 把网络序IPv4地址转化为字符串
 *@ip: 网络序IPv4地址; @out: 输出IP字符串;@osize: out空间大小(sizeof)
 */
const char * net_to_str(unsigned int ip, char *out, int osize)
{
	return inet_ntop(AF_INET, &ip, out, osize);
}
...
get_ext_iface(void)
{
	...
	while(keep_detecting) {
		...
		while (!feof(input)) {
			...
			if (strcmp(gw, "00000000") == 0) {
				...
				fclose(input); // add by lijg, 2013-05-27, this is a BUG, not close it
				return device;
			}
		}
		fclose(input);
	...
	int is_online() {
		// modified by lijg, 2013-07-17,  while after NTP , @last_offline_time 's value is dangerous 
	...
	char * get_status_text() {
		...
		count = 0;
		while (first != NULL && count < 50 ) {  // modified by lijg, 2013-05-28, 超过最大值的话buffer会溢出,最多打印50个客户端信息
		...
		// modified by lijg, 2013-05-28, only show the first auth server for avoiding LOCK_CONFIG .
		//LOCK_CONFIG();

		for (auth_server = config->auth_servers; auth_server != NULL; auth_server = auth_server->next) {
			snprintf((buffer + len), (sizeof(buffer) - len), "  Host: %s (%s)\n", auth_server->authserv_hostname, auth_server->last_ip);
			len = strlen(buffer);
			break; // add by lijg, 2013-05-28
		}
