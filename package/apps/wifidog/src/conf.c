...
config_init(void)
{
	...
	// modified by lijg, 2013-05-18, 打印调试日志
...
*///called by config_read(),解析配置文件中一行中(例如"GatewayInterface br-lan")的 "GatewayInterface"
//  对应的 opcode, 即=oGatewayInterface
static OpCodes
...
*///called by config_read() 's "oAuthServer", @filename="wifidog.conf"
// 解析wifidog.conf中的配置段"AuthServer",即认证服务其相关的配置
static void
parse_auth_server(FILE *file, const char *filename, int *linenum)
{
	...
	loginscriptpathfragment = safe_strdup(DEFAULT_AUTHSERVLOGINPATHFRAGMENT);// "login/?"
...
config_read(const char *filename)
{
	...
	while (!feof(fd) && fgets(line, MAX_BUF, fd)) { //循环读取配置文件中的每一行
		linenum++;
		s = line;
		...
		if (p1 && p1[0] != '\0') {
			...
			if ((strncmp(s, "#", 1)) != 0) {
				debug(LOG_DEBUG, "Parsing token: %s, "
						"value: %s", s, p1);
				opcode = config_parse_token(s, filename, linenum);//解析配置项对应的opcode
