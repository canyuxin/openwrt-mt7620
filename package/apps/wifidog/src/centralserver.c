...
auth_server_request(t_authresponse *authresponse, const char *request_type, const char *ip, const char *mac, const char *token, unsigned long long int incoming, unsigned long long int outgoing)
{
	...
	sockfd = connect_auth_server(); // 连接认证服务器的80端口(默认端口)
	if (sockfd == -1) {
	...
	do {
		...
		timeout.tv_sec = 10; /* XXX magic... 30 second is as good a timeout as any */ // modified by lijg, 2013-06-24
		timeout.tv_usec = 0;
		nfds = sockfd + 1;
 ...
 */
// called by auth_server_request(),TCP连接认证服务器
int connect_auth_server() {
	int sockfd;

	LOCK_CONFIG();
	sockfd = _connect_auth_server(0); // 从认证服务器列表开始连接每个服务器直到有一个成功为止
	UNLOCK_CONFIG();
 ...
 */
// 1. called by connect_auth_server(), TCP连接认证服务器并更新认证服务器的防火墙规则
//  @level :从零开始, 逐一连接每个认证服务器,直到有一个连接成功为止
//  同时将坏的认证服务器移到@config->auth_servers 链表尾部
//  1.1 获取认证服务器的域名成功
int _connect_auth_server(int level) {
	s_config *config = config_get_config();
	t_auth_serv *auth_server = NULL;
	struct in_addr *h_addr;
	int num_servers = 0; // 认证服务器个数
	char * hostname = NULL;
	...
	if (auth_server->last_ip) { // add by lijg, 2013-08-20, get dns cache for auth svr 
		h_addr = safe_malloc(sizeof(struct in_addr));
		if (inet_aton(auth_server->last_ip, h_addr) == 0) {
			free(h_addr);
			h_addr = NULL;
		}
		debug(LOG_DEBUG, "get auth server %s directly %p", auth_server->last_ip, h_addr);
	} else {
	...
	if (!h_addr) {
		...
		if (h_addr) {
			...
			mark_auth_server_bad(auth_server);  // 将这个认证服务器放到链表尾部
			return _connect_auth_server(level); // 连接下一个认证服务器
		}
		else {
			/*
			 * No
		...
		if (!auth_server->last_ip || strcmp(auth_server->last_ip, ip) != 0) { //1.2 如果认证服务器的IP地址已经变化了 则更新防火墙规则
			/*
			...
			// modified by lijg, 2013-05-30, don't clear auth server firewall rules
			//fw_clear_authservers(); // 清理认证服务器的防火墙规则
			if (!iptables_fw_find_mention("filter", TABLE_WIFIDOG_AUTHSERVERS, ip)) {
				fw_set_authservers(); // 确保 不要添加重复的规则
			}
		}
		else {
			/*
